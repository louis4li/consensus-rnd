...
| `$MANAGED_WORK_USER_SCOPE_ENABLE` | defaulted | shared ManagedWorkSnapshot current-user routing scope | `true` | missing, empty, or true-like values `true`, `1`, `yes`, or `on` restrict shared open managed work discovery to issues authored by or assigned to the current `gh api user` login, plus PRs with exactly one `Closes #N` link to an in-scope issue; false-like values `false`, `0`, `no`, or `off` keep all open managed work routable; current-login read failure makes the shared snapshot fail closed so daemon consumers noop and wakeup-plan emits a status-only unavailable action | managed_work_snapshot, wakeup plan, phase9-router, patrol-inspector, comment-monitor | `test_managed_work_snapshot.py`, `test_wakeup_plan.py`, `test_host_env_surface_matrix.py`, `test_skill_reference_anchors.py` |
